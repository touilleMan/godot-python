...
RESERVED_NAMES = ('class', 'short', 'long', 'default', 'template', 'char', 'export', 'bool')
...
"""
HEADER = HEADER_HEAD + HEADER_TAIL
FOOTER = \
"""
...
def api_ordered_by_dependencies(api):
    ordered_api = []
    defs = []
    for clsapi in api:
        defs.append((clsapi['name'], clsapi['base_class'], clsapi))
    ...
    while parents:
        ordered_api += [v for _, _, v in parents]
        children = []
        for p in parents:
...
def generate_classes_def(api):
    ...
    for clsapi in api:
        output.append(generate_class_def(clsapi))
    output.append('')
    return '\n'.join(output) + '\n'
...
def _mk_const_name(basename):
    return '__const__' + _strip_dummy_function_name(basename)
...
def generate_class_def(clsapi):
    ...
    if not clsapi['base_class']:
        # Root class like Object get special treatment
        output.append("""struct {name} {{
    ::Object *godot_obj;
    ::Variant godot_variant;

    explicit {name}(::StringName name) : {name}(ClassDB::instance(name)) {{
    }}
    ...
    {name}() : {name}(::StringName("{name}")) {{ }}
    explicit {name}(::Object *obj) {{
        this->godot_obj = obj;
        this->godot_variant = ::Variant(obj);
    }}
    explicit {name}(::Variant var) {{
        this->godot_obj = var.operator ::Object*();
        this->godot_variant = var;
    }}
""".format(**clsapi))
    else:
        output.append('struct {name} : public {base_class} {{'.format(**clsapi))
        output.append("""
    explicit {name}(::Object *obj) : {base_class}(obj) {{ }}
    explicit {name}(::Variant var) : {base_class}(var) {{ }}
    explicit {name}(::StringName name) : {base_class}(name) {{ }}
    {name}() : {base_class}(::StringName("{name}")) {{ }}
""".format(**clsapi))
    ...
    output.append('    // Bind constants')
    for key, value in clsapi['constants'].items():
        output.append('    const int {keyname} = {value};'.format(value=value, keyname=_mk_const_name(key)))
...
def generate_class_impl(clsapi):
    ...
    def _build_method_impl(clsname, methodapi):
        ...
        # TODO: Replace this once api.json handles VMethod yet
        output.append('if (!__mb) %s;' % ret_from_variant(methodapi['return_type'], 'Variant()'))
...
def generate_binding_class(clsapi):
    ...
    def _cook_method_params(methodapi):
        ...
        for a in methodapi['arguments']:
            ...
            # NOT_SUPPORTED_DEFAULTS = ('Transform', 'Transform2D', 'RID', 'Object', 'Texture')
            # if dv and a['type'] not in NOT_SUPPORTED_DEFAULTS or a['type'].startswith('['):
            #     if a['type'] == 'String':
            #         param += '="%s"' % dv
            #     elif a['type'] == 'Vector2':
            #         param += '=Vector2%s' % dv
    ...
    #     output.append('.def("__new__", [](py::object cls) -> py::object { py::eval("raise TypeError(\'%s is not instanciable.\')"); return py::object(); }, py::arg("cls"))' % clsname)
    output.append('.def("__set_godot_obj", &%s::set_godot_obj)' % clsname)
    ...
    for key, value in clsapi['constants'].items():
        ...
        output.append('.def_readonly("{key}", &{clsname}::{keyname})'.format(
            key=key, clsname=clsname, keyname=_mk_const_name(key)))
...
def main(infd, output, split=False):
    api = json.load(infd)
    api = api_ordered_by_dependencies(api)
    if split:
        generate_splitted(api, output)
    else:
        generate_united(api, output)


def generate_united(api, outfile):
    output = HEADER
    output += '\n\n'
...
def generate_splitted(api, outdir):
    if outdir.endswith('.cpp'):
        outdir = outdir[:-4]
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    for old in os.listdir(outdir):
        os.unlink('%s/%s' % (outdir, old))
    # Generate header
    with open('%s/bindings.gen.h' % outdir, 'w') as fd:
        fd.write(HEADER)
        fd.write(generate_forward_classes_def(api))
        for clsapi in api:
            fd.write('\n\n')
            fd.write(generate_class_def(clsapi))
            fd.write("""
void register_{name}_binding(py::module m);
""".format(**clsapi))
        fd.write('\n\n')
        fd.write(FOOTER)
    # Generate main src file
    with open('%s/bindings.gen.cpp' % outdir, 'w') as fd:
        fd.write(HEADER_HEAD)
        fd.write('\n#include "bindings.gen.h"\n')
        fd.write(HEADER_TAIL)
        fd.write('\n\n')
        fd.write("""
void __register_generated_bindings(py::module m) {
""")
        for clsapi in api:
            fd.write("    register_{name}_binding(m);\n".format(**clsapi))
        fd.write("""
...
""")
        fd.write(FOOTER)
    # Generate src file for each godot class
    for clsapi in api:
        with open('%s/binding_%s.cpp' % (outdir, clsapi['name']), 'w') as fd:
            fd.write(HEADER_HEAD)
            fd.write('\n#include "bindings.gen.h"\n')
            fd.write(HEADER_TAIL)
            fd.write(generate_class_impl(clsapi))
            fd.write('\n\n')
            fd.write("""
void register_{name}_binding(py::module m) {{
""".format(**clsapi))
            fd.write(generate_binding_class(clsapi))
            fd.write('\n\n')
            fd.write("""
}
""")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', '-o', default='bindings.gen.cpp',
                        help='Generated output (default: bindings.gen.cpp)')
    parser.add_argument('--split', '-s', action='store_true',
                        help='Generate output as a folder for faster compilation.')
    args = parser.parse_args()
    main(args.input, args.output, args.split)
