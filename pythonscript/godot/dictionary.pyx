...
cdef class Dictionary:
    ...
    cdef inline godot_bool operator_equal(self, Dictionary other):
        return gdapi10.godot_dictionary_operator_equal(
    ...
    cdef inline godot_bool operator_contains(self, object key):
        ...
        cdef godot_bool ret = gdapi10.godot_dictionary_has(&self._gd_data, &var_key)
    ...
    cdef inline void operator_setitem(self, object key, object value):
        ...
        gdapi10.godot_dictionary_set(&self._gd_data, &var_key, &var_value)
        gdapi10.godot_variant_destroy(&var_key)
    ...
    cpdef inline godot_int hash(self):
        return gdapi10.godot_dictionary_hash(&self._gd_data)

    cpdef inline godot_int size(self):
        return gdapi10.godot_dictionary_size(&self._gd_data)
    ...
    cpdef inline void clear(self):
        gdapi10.godot_dictionary_clear(&self._gd_data)

    cpdef inline godot_bool empty(self):
        return gdapi10.godot_dictionary_empty(&self._gd_data)

    cpdef inline godot_bool has_all(self, Array keys):
        return gdapi10.godot_dictionary_has_all(&self._gd_data, &keys._gd_data)

    cpdef inline void erase(self, object item):
        cdef godot_variant var_item
        pyobj_to_godot_variant(item, &var_item)
        gdapi10.godot_dictionary_erase(&self._gd_data, &var_item)
        gdapi10.godot_variant_destroy(&var_item)

    # TODO: would be better to turn this into an iterator
    cpdef inline list keys(self):
        cdef godot_array gd_keys = gdapi10.godot_dictionary_keys(&self._gd_data)
        cdef int i
        cdef list ret = [
            godot_variant_to_pyobj(gdapi10.godot_array_operator_index(&gd_keys, i))
            for i in range(gdapi10.godot_array_size(&gd_keys))
        ]
        gdapi10.godot_array_destroy(&gd_keys)
        return ret

    # TODO: would be better to turn this into an iterator
    cpdef inline list values(self):
        cdef godot_array gd_values = gdapi10.godot_dictionary_values(&self._gd_data)
        cdef int i
        cdef list ret = [
            godot_variant_to_pyobj(gdapi10.godot_array_operator_index(&gd_values, i))
            for i in range(gdapi10.godot_array_size(&gd_values))
        ]
        gdapi10.godot_array_destroy(&gd_values)
        return ret

    cpdef inline str to_json(self):
        cdef godot_string var_ret = gdapi10.godot_dictionary_to_json(&self._gd_data)
        cdef object ret = godot_string_to_pyobj(&var_ret)
        gdapi10.godot_string_destroy(&var_ret)
        return ret
