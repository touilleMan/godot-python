...
vars.Add(
    BoolVariable(
        "symlink_cp_fallback", "Symlink are poorly supported on Windows, fallback to copy instead", False
    )
)
...
def SymLink(target, source, env):
    ...
    if env["symlink_cp_fallback"]:
        ...
        try:
            ...
            raise UserError(f"Can't do copy as symlink fallback ({abs_src} -> {abs_trg}): {e}")
