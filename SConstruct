...
def cython_compile(env, source):
    ...
    cflags = env["CFLAGS"] if env["shitty_compiler"] else f"{env['CFLAGS']} -Wno-unused"
    return env.SharedLibrary(libs, source, LIBPREFIX="", SHLIBSUFFIX=suffix, CFLAGS=cflags)
...
gdnative_api_struct_pxd = File("pythonscript/godot_hazmat/gdnative_api_struct.pxd")
...
godot_hazmat_pxd_deps = [*env.Glob("*pythonscript/godot_hazmat/*.pxd"), gdnative_api_struct_pxd]
godot_pxd_deps = [*env.Glob("pythonscript/godot/*.pxd"), *godot_hazmat_pxd_deps, godot_bindings_pxd]


### Collect and build `pythonscript/godot_hazmat` module ###


# Special handling for _bootstrap.pyx given it generates _bootstrap_api.h header

pythonscript_godot_hazmat__bootstrap_pyx = "pythonscript/godot_hazmat/_bootstrap.pyx"
pythonscript_godot_hazmat__bootstrap_c_scrs = env.CythonToC(
    source=pythonscript_godot_hazmat__bootstrap_pyx,
    target=("pythonscript/godot_hazmat/_bootstrap.c", "pythonscript/godot_hazmat/_bootstrap_api.h"),
)
env.Depends(pythonscript_godot_hazmat__bootstrap_c_scrs, godot_hazmat_pxd_deps)
env.Depends(pythonscript_godot_hazmat__bootstrap_c_scrs, env.Glob("pythonscript/_bootstrap_*.pxi"))
pythonscript_godot_hazmat__bootstrap_c, pythonscript_godot_hazmat__bootstrap_api_h, *_ = pythonscript_godot_hazmat__bootstrap_c_scrs
pythonscript_godot_hazmat__bootstrap_targets = env.CythonCompile(source=[pythonscript_godot_hazmat__bootstrap_c])

# Now compile the other *.pyx

pythonscript_godot_hazmat_pyx_compiled = [
    *[
        env.Cython(src)
        for src in env.Glob("pythonscript/godot_hazmat/*.pyx")
        if src != pythonscript_godot_hazmat__bootstrap_pyx
    ],
    *pythonscript_godot_hazmat__bootstrap_targets
]
env.Depends(pythonscript_godot_hazmat_pyx_compiled, godot_hazmat_pxd_deps)
pythonscript_godot_hazmat_targets = [
    *env.Glob("pythonscript/godot_hazmat/*.py"),
    *env.Glob("pythonscript/godot_hazmat/*.pxd"),
    *pythonscript_godot_hazmat_pyx_compiled,
]
...
env.Depends("pythonscript/pythonscript.c", pythonscript_godot_hazmat__bootstrap_api_h)
...
def generate_build_dir(target, source, env):
    ...
    godot_hazmat_module = source[2]
    godot_module = source[3]
    ...
    env["add_pythonscript_stuff_to_build_dir"](env, target, libpythonscript, godot_hazmat_module, godot_module)
