...
generate_gdnative_api_struct = env.Command(
    ...
    action=(
        "autopxd -I ${SOURCES[0]} ${SOURCES[1]} > ${TARGET}"
    ),
)
...
godot_bindings_pyx = env.Command(
    ...
    action=(
        "python tools/generate_bindings.py  -i ${SOURCES} -o ${TARGET}"
    ),
)
...
# `bindings.pyx` is autogenerated after the glob
pythonscript_godot_pyx_srcs = [*env.Glob("pythonscript/godot/*.pyx"), "pythonscript/godot/bindings.pyx"]
pythonscript_godot_pyx_compiled = [
    env.Cython(src) for src in pythonscript_godot_pyx_srcs
]
...
pythonscript_godot_targets = [
    ...
    *[
        target
        for src in pythonscript_godot_pyx_srcs
        for target in env.Cython(src)
    ],
]
...
env.Depends(pythonscript__godot_c_scrs, godot_bindings_pyx)
...
pythonscript__godot_targets = env.CythonCompile(source=[pythonscript__godot_c])
...
env.Append(CFLAGS="-Wno-unused")  # Needed to compile autogenerated Cython code
